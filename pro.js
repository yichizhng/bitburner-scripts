...
function pickTarget(ns) {
	...
	return getAllServers(ns)
		...
		.map(s => {
			...
			return [s.hostname,
			// TODO: consider preferring prepped servers
...
function launchHack(ns, target, threads, ramMap, pids) {
	...
	let hs = ramMap.filter(x => x[1] >= threads * 1.7).reduce((a, b) => b[2] > a[2] ? a : b);
	if (!hs) return false;
	hs[1] -= 1.7 * threads;
	if (!pids) return true;
...
/**
 * Note that this function is not actually going to get used, because the logic around launching grows
 * on cored servers is more complicated than what this function can handle.
 * @param {NS} ns
 * @param {string} target The server to target.
 * @param {string} threads The number of adjusted grow threads; the actual threads used may
 * be lower if scheduled on a server with cores.
 * @param {[string, number, number][]} ramMap
 * @param {number[]} [pids] Array to append the pid to or undefined; if undefined, runs in dry mode.
 * @returns {boolean} Whether the grow was successfully launched, or
 * scheduled if in dry mode.
 */
function launchGrow(ns, target, threads, ramMap, pids) {
	// Prefer cored ram for grow
	let gs = ramMap.filter(x => x[1] >= threads * 1.75).reduce((a, b) => b[2] > a[2] ? b : a);
	if (!gs) return false;
	let at = Math.ceil(threads / ((15 + gs[2]) / 16));
	gs[1] -= 1.75 * at;
	if (!pids) return true;
	let pid = ns.exec('grow-once.js', gs[0], { threads: at, temporary: true }, target, ns.getWeakenTime(target) - ns.getGrowTime(target));
	if (pid) {
		pids.push(pid);
		return true;
	}
...
function launchWeaken(ns, target, threads, ramMap, minCores, pids) {
	while (threads > 0) {
		...
		let ws = ramMap.filter(x => x[1] >= 1.75 && x[2] >= minCores).reduce((a, b) => b[2] > a[2] ? a : b);
		if (!ws) return false;
  ...
  while (true) {
    ...
    schedule_loop: while (batches_launched < BATCH_CAP) {
      ...
      growServers.forEach(s=>{s[2] = 1});
		...
		ns.print(`Expected hacking xp: ${po.exp.hacking}`)
    await 0; await 0; await ns.asleep(ns.getWeakenTime(target));
		if (server.hackDifficulty > server.minDifficulty) {
			ns.tprint('Server above min diff; bug in the script, or hacknet boosting?');
		}
		if (server.moneyAvailable < server.moneyMax) {
			ns.tprint(`Desync detected; money left ${server.moneyAvailable}/${server.moneyMax}`);
		}
		ns.print(`New hacking xp: ${ns.getPlayer().exp.hacking}`)
    // await ns.sleep(0);
  }
